  ...
  <title>后台管理</title>
    ...
    <script type="text/javascript" src="/myblog/Public/ueditor/ueditor.all.js"></script>
       ...
       function Delete(id){
        ...
        if(confirm_){
          $.ajax({
            url : "<?php echo U('Admin/CatManage/deleteCat');?>",
            data : {
              'id' : id
            },
            type : 'post',
            success : function(data){
                    if(data[0]==1){
                      alert('删除成功！');
                      window.location.href = "<?php echo U('Admin/CatManage/index');?>";
                    }else {
                      alert('删除失败！');
                    }
            }
...
<nav class="navbar navbar-default navbar-fixed-top navbar-inverse" role="navigation">
  ...
  <div class="panel-heading">
   <h3 class="panel-title">当前位置：<span class="glyphicon glyphicon-home"></span><a href="<?php echo U('Admin/Adm/index');?>">首页</a>>分类管理</h3>
  </div>
  <div class="panel-body">
    <table class="table table-hover text-center table-condensed">
    	<tr><td>分类ID</td><td>分类名</td><td>分类图片</td><td>文章数量</td><td>管理</td></tr>
      ...
      <td><img style="width: 50px;height: 30px;" src="/myblog/Public/Uploads/<?php echo ($vo["img_path"]); echo ($vo["img_name"]); ?>"></td><td><?php echo ($vo["article_sum"]); ?></td>
      <td><a class="btn btn-danger" id="<?php echo ($vo["id"]); ?>" href="javascript:Delete(<?php echo ($vo["id"]); ?>);">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
    </table>
    </div>
   </div>
  </div>
