...
/*对应分类信息*/
        $cate_info = $cate -> where("name = '$cateName'")->select();
/*最热文章信息*/          
...
/*对应类文章分页*/
        $where="id>= 1 and category = '$cateName'";
        $count=$article->where($where)->count();
        $p=getpage($count,6);
        $list=$article->where($where)->order('create_time desc')->limit($p->firstRow,$p->listRows)->select();
